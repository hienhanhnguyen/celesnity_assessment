...
import { Batch, CanonicalEvent, ManagementEvent } from '../database/entities';
import type {
  BatchRecord,
  CanonicalRecord,
  ManagementRecord,
...
@Injectable()
export class TypeOrmProductionStore implements ProductionStore {
  ...
  async loadManagementEvents(): Promise<ManagementRecord[]> {
    const events = await this.dataSource.getRepository(ManagementEvent).find();
    return events.map((event) => ({
      id: event.id,
      batchId: event.batchId,
      type: event.type as ManagementEventType,
      actor: event.actor,
